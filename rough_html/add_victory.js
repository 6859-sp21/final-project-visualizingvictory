const flattenName = (name) => {
		...
		const libertyColor = "rgb(47, 50, 56)";
		const victoryColor = "rgb(3, 69, 252)"
        ...
        const data2Text = (shipData) =>{
			let text = `Name: ${shipData.Name}<br/>    Date Completed: ${shipData.Completed}`
			//fate: ${shipData.Fate}
			return(text);
        }
        ...
        map.selectAll("circle.shipYard")
			...
			.on('mouseover', function (d, i) {
				console.log(`#${flattenName(i.properties.name)}Bar`);
				d3.select(`#${flattenName(i.properties.name)}Bar`)
			...
			d3.select(this)
				.attr("fill",shipYardColorOff)
				.attr("r","4px");
				d3.selectAll(`#${flattenName(i.properties.name)}Bar`)
				.attr("fill",shipYardColorOff);
				})
		...
		.on('mouseover', function (d, i) {
			...
			d3.select(`#${flattenName(i.properties.name)}`)
			.attr("fill",selectColor)
			.attr("r","8px");
		})
		.on('mouseout', function (d, i) {
			d3.select(this)
			.style("fill",shipYardColorOff)
			.attr("id",function(d){return(`${flattenName(d.properties.name)}Bar`)});
			d3.select(`#${flattenName(i.properties.name)}`)
			.attr("fill",shipYardColorOff)
			.attr("r","4px");
